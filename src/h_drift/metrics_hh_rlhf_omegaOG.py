...
Attach WWWWHW (Ω) features to the HH-RLHF H-drift dataset and
print basic summaries:

- how often each Ω dimension is dominant
- mean H_drift_index by Ω-dominant dimension
- WHY vs HOW comparison (as a first cube-geometry sanity check)
"""
...
TEXT_COL = "response"  # HH-RLHF answer text column
...
def compute_omega_row(text: str) -> pd.Series:
    """
    Given a text, return a Series with:
      omega_who, ..., omega_how,
      omega_entropy,
      omega_total_markers,
      omega_dominant_dim
...
def main() -> None:
    ...
    if TEXT_COL not in df.columns:
        raise RuntimeError(f"Expected text column '{TEXT_COL}' not found in data.")

    # Attach Ω features
    print(f"Computing Ω features from column: '{TEXT_COL}' ...")
    ...
    print("\n=== Ω dominant dimension counts (all rows) ===")
    print(df_omega["omega_dominant_dim"].value_counts(dropna=False))

    # Mean H_drift by Ω-dominant dimension
    ...
    if LABEL_COL in df_omega.columns and HDRIFT_COL in df_omega.columns:
        print("\n=== Mean H_drift_index by (label, Ω-dominant dimension) ===")
        grouped = (
